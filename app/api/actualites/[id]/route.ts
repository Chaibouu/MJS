...
import { updateActualiteSchema } from "@/schemas/actualite";
import {
  uploadActualiteMainImage,
  uploadActualiteGalleryImage,
  deleteActualiteGalleryImage,
} from "@/lib/upload";
...
export async function GET(
  ...
  try {
    ...
    const a = await db.actualite.findFirst({
    ...
    return a
      ? NextResponse.json(a)
      : NextResponse.json({ error: "Actualité non trouvée" }, { status: 404 });
  } catch (e) {
    console.error("GET /api/actualites/[id]:", e);
...
export async function PUT(
  ...
  try {
    ...
    const { id } = await params;
    const existing = await db.actualite.findFirst({
      where: { id, isDeleted: false },
    });
    if (!existing) {
    ...
    const parsed = updateActualiteSchema.safeParse({
    ...
    if (mainImage && mainImage.size > 0) {
      const res = await uploadActualiteMainImage(mainImage, id);
      imagePath = res.path;
    }
    ...
    for (const p of galleryRemove) {
      await deleteActualiteGalleryImage(p);
    }

    const added: string[] = [];
    for (const f of galleryAdd) {
      const g = await uploadActualiteGalleryImage(f, id);
      added.push(g.path);
    }
    ...
    const updated = await db.actualite.update({
      where: { id },
      data: data as Parameters<typeof db.actualite.update>[0]["data"],
      include: {
        createdByUser: {
          select: { id: true, name: true, email: true, firstName: true, lastName: true },
        },
    ...
    console.error("PUT /api/actualites/[id]:", e);
...
export async function DELETE(
  ...
  try {
    ...
    console.error("DELETE /api/actualites/[id]:", e);
    return NextResponse.json(
