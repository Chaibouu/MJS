...
export async function PUT(
  ...
  try {
    ...
    if (mainImage && mainImage.size > 0) {
      const res = await uploadArticleMainImage(mainImage, id);
      imagePath = res.path;
    }
    ...
    for (const p of galleryRemove) {
      await deleteArticleGalleryImage(p);
    }

    const added: string[] = [];
    for (const f of galleryAdd) {
      const g = await uploadArticleGalleryImage(f, id);
      added.push(g.path);
    }
    ...
    const updated = await db.article.update({
      where: { id },
      data: data as Parameters<typeof db.article.update>[0]["data"],
      include: {
        createdByUser: {
          select: { id: true, name: true, email: true, firstName: true, lastName: true },
        },
